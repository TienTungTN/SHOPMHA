		...
		<div class="footer">
			<div class="grid wide">
							...
							if($ok == 2)
					{
						...
						<td data-th="Price">'.number_format($row['giasp'],3).'đ</td> 
						<td data-th="Quantity">
						<input class="form-control text-center" type="number" name="qty['.$row['id'].']" value="'.$_SESSION['cart'][$row['id']].'">
						</td> 
						<td data-th="Subtotal" class="text-center">'.number_format($_SESSION['cart'][$row['id']]*($row['giasp']),3).' đ</td> 
					...
					if ($_SERVER['REQUEST_METHOD'] == 'POST') {
						foreach ($_POST['qty'] as $id => $qty) {
							// Kiểm tra và đảm bảo số lượng lớn hơn 0
							if ($qty > 0) {
								$_SESSION['cart'][$id] = $qty;
							} else {
								$_SESSION['cart'][$id] = 1; // Hoặc bạn có thể xóa sản phẩm khỏi giỏ hàng
							}
						}
						// Cập nhật giỏ hàng và thực hiện các thao tác cần thiết
					}
